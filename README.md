...
12-28和12-31的大作业现场报告，报过名的组别先开始，未报名组别可能需要到第二次才开始。无论对于那一类组别，报告顺序均由Python程序随机生成。被抽到的小组，请上台做报告，待同学们投票结束后，按Python程序提示，统计出自己的成绩。之后，请按Python程序提示，选择下一个做报告的组。
...
是否展示        是
Name: ¥¥¥, dtype: object
本组演示结束后，请同学们发送自己的评分到
lecture_sysu@163.com，
 标题格式为: 
&&-分数
分数取整，0=<分数<=100，不用写邮件正文

统计成绩的提示如下：

按任意键显示本组成绩(按'q'退出程序):>? 
本组同学共收到2封打分email
第&&组同学得到的平均分95.0, 标准差为7.0710678118654755

关于评分：
每个组的报告结束之后，请全体同学，进行一次email打分。请用中大官邮或者授课教师确认过的替代邮箱，发打分email到lecture_sysu@163.com。打分信息只需要体现在邮件标题上，标题格式为：组别-分数。

打分区间为0-100，请根据自己对现场报告的判断给出整数分。考虑到一个人独立完成大作业的同学需要付出更大的勇气和精力，请大家考虑适当提高对其所做的评分，比如不考虑人数情况下打80分，考虑的话，可以上浮百分之20，达到96分（举例而已，具体情况大家自己把握）。

打分代码在Mini-Project文件夹中，有兴趣的可以检查一下是否可以做到现场匿名：）。
